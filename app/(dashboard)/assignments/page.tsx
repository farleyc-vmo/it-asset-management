...
export default function AssignmentsPage() {
  const { assetAssignments, setAssetAssignments, assetStocks, assets, employees, warehouses } = useData()
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case 'assigned': return 'bg-green-500/10 text-green-500'
      case 'pending': return 'bg-amber-500/10 text-amber-500'
      case 'returned': return 'bg-blue-500/10 text-blue-500'
  ...
  const handleSave = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const assignmentData: AssetAssignment = {
      ...
      assignment_type: formData.get('assignment_type') as AssetAssignment['assignment_type'],
      ...
      serials: (formData.get('serials') as string).split(',').map(s => s.trim()).filter(Boolean),
    ...
    if (editing) {
      setAssetAssignments(prev => prev.map(a => a.id === editing.id ? assignmentData : a))
    } else {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Track asset assignments to employees</p>
        </div>
            ...
            <Table>
              ...
              <TableBody>
                {filteredAssignments.map(assignment => {
                  const employee = employees.find(e => e.id === assignment.assigned_to)
                  ...
                  return (
                    <TableRow key={assignment.id}>
                      ...
                      <TableCell>
                        <div>
                          <p className="text-sm">{assetInfo?.asset?.asset_name || '-'}</p>
                      ...
                      <TableCell className="text-right">
                        <Button variant="ghost" size="icon" onClick={() => openView(assignment)}>
          ...
          <form onSubmit={handleSave} className="space-y-4">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="space-y-2">
                ...
                <Select name="stock_id" defaultValue={editing?.stock_id} required>
                  <SelectTrigger>
                    ...
                    {assetStocks.map(stock => {
                      ...
                      return (
                        <SelectItem key={stock.id} value={stock.id}>
                          {asset?.asset_name} - {warehouse?.name} (Avail: {stock.available_quantity})
                        </SelectItem>
                      )
                    })}
                  </SelectContent>
              ...
              <div className="space-y-2">
                ...
                <Select name="assigned_to" defaultValue={editing?.assigned_to} required>
                  <SelectTrigger>
              ...
              <div className="space-y-2">
                ...
                <Select name="assignment_type" defaultValue={editing?.assignment_type || 'permanent'}>
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
              ...
              <div className="space-y-2">
                ...
                <Input id="quantity" name="quantity" type="number" min="1" defaultValue={editing?.quantity || 1} required />
              </div>
              <div className="space-y-2">
                <Label htmlFor="assigned_date">Assigned Date</Label>
                <Input id="assigned_date" name="assigned_date" type="date" defaultValue={editing?.assigned_date || new Date().toISOString().split('T')[0]} required />
              </div>
              <div className="space-y-2">
                <Label htmlFor="expected_return_date">Expected Return Date</Label>
                <Input id="expected_return_date" name="expected_return_date" type="date" defaultValue={editing?.expected_return_date || ''} />
              </div>
              <div className="space-y-2">
                <Label htmlFor="condition_before">Condition Before</Label>
                <Input id="condition_before" name="condition_before" defaultValue={editing?.condition_before || 'Good'} required />
              </div>
              <div className="space-y-2">
                <Label htmlFor="condition_after">Condition After</Label>
                <Input id="condition_after" name="condition_after" defaultValue={editing?.condition_after || ''} />
              </div>
              <div className="space-y-2">
                <Label htmlFor="returned_date">Returned Date</Label>
                <Input id="returned_date" name="returned_date" type="date" defaultValue={editing?.returned_date || ''} />
              </div>
              <div className="space-y-2">
                <Label htmlFor="approved_by">Approved By</Label>
                <Select name="approved_by" defaultValue={editing?.approved_by || ''}>
                  <SelectTrigger>
            ...
            <div className="space-y-2">
              ...
              <Input id="serials" name="serials" defaultValue={editing?.serials.join(', ')} placeholder="SN001, SN002" />
            </div>
            <div className="space-y-2">
              <Label htmlFor="note">Note</Label>
              <Textarea id="note" name="note" defaultValue={editing?.note} rows={3} />
            </div>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4 text-sm">
                ...
                <div>
                  ...
                  <Badge className={getPriorityColor(viewing.priority)}>{viewing.priority}</Badge>
                </div>
                <div>
                  <p className="text-muted-foreground">Type</p>
                  <p className="font-medium capitalize">{viewing.assignment_type}</p>
                </div>
                <div>
                  <p className="text-muted-foreground">Assigned To</p>
                  <p className="font-medium">{employees.find(e => e.id === viewing.assigned_to)?.full_name}</p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p className="font-medium">{new Date(viewing.assigned_date).toLocaleDateString()}</p>
                </div>
                <div>
                  <p className="text-muted-foreground">Expected Return</p>
                  <p className="font-medium">{viewing.expected_return_date ? new Date(viewing.expected_return_date).toLocaleDateString() : '-'}</p>
                </div>
