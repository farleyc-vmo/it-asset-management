...
export default function AssetsPage() {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'deployed': return 'bg-blue-500/10 text-blue-500'
      case 'in_stock': return 'bg-cyan-500/10 text-cyan-500'
  ...
  const handleSave = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (editing) {
      setAssets(prev => prev.map(a => a.id === editing.id ? assetData : a))
    } else {
  ...
  const handleDelete = (id: string) => {
    if (confirm('Are you sure you want to delete this asset?')) {
      setAssets(prev => prev.map(a => a.id === id ? { ...a, deleted_at: new Date().toISOString() } : a))
    }
  }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Manage your IT assets inventory</p>
        </div>
                ...
                {filteredAssets.map(asset => {
                  ...
                  return (
                    <TableRow key={asset.id}>
                      ...
                      <TableCell>
                        {new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(asset.purchase_cost)}
                      </TableCell>
          ...
          <form onSubmit={handleSave} className="space-y-4">
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="space-y-2">
                ...
                <Input id="asset_name" name="asset_name" defaultValue={editing?.asset_name} required />
              </div>
              <div className="space-y-2">
              ...
              <div className="space-y-2">
                ...
                <Input id="purchase_date" name="purchase_date" type="date" defaultValue={editing?.purchase_date} required />
              </div>
              <div className="space-y-2">
                <Label htmlFor="purchase_cost">Purchase Cost (VND)</Label>
                <Input id="purchase_cost" name="purchase_cost" type="number" defaultValue={editing?.purchase_cost} required />
              </div>
              <div className="space-y-2 sm:col-span-2">
                <Label htmlFor="specification">Specification</Label>
                <Input id="specification" name="specification" defaultValue={editing?.specification} />
              </div>
              <div className="space-y-2 sm:col-span-2">
                <Label htmlFor="supplier_url">Supplier URL</Label>
                <Input id="supplier_url" name="supplier_url" type="url" defaultValue={editing?.supplier_url} />
              </div>
              <div className="space-y-2 sm:col-span-2">
